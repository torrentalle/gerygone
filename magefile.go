...
const (
	packageName  = "github.com/torrentalle/gerygone"
...
// Install Go Dep and sync Gerygone's vendored dependencies
func Vendor() error {
	mg.Deps(getDep)
	return sh.Run("dep", "ensure")
}

// Gerygone gerygone binary
func Gerygone() error {
	return sh.RunWith(flagEnv(), goexe, "build", "-ldflags", ldflags, packageName)
}

// Gerygone gerygone binary with race detector enabled
func GerygoneRace() error {
	return sh.RunWith(flagEnv(), goexe, "build", "-race", "-ldflags", ldflags, packageName)
}

// Install gerygone binary
func Install() error {
...
// Gerygone gerygone without git info
func GerygoneNoGitInfo() error {
...
var pkgPrefixLen = len("github.com/torrentalle/gerygone")

func gerygonePackages() ([]string, error) {
	mg.Deps(getDep)
